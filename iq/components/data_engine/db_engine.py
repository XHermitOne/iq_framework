...
class iqDBEngineManager(object):
        ...
        try:
            ...
            try:
                ...
                if result and result.returns_rows:
                    ...
                    recordset = [{name: float(value) if isinstance(value, decimal.Decimal) else value for name, value in rec._asdict().items()} for rec in records]
                else:
    ...
    def getSessionClass(self, db_url=None, base=None, *args, **kwargs):
        ...
        if not hasattr(self, DEFAULT_SESSION_CLASS_ATTR_NAME):
            ...
            base.metadata.create_all(biind=engine, checkfirst=True)
