...
from iq.util import icon_func
from iq.engine.wx import wxbitmap_func

# To manage interaction with wxPython controls, a form manager is used
from iq.engine.wx import form_manager
from iq.engine.wx import listctrl_manager

__version__ = (0, 0, 0, 1)

UNKNOWN_STATE_NAME_FMT = u'State %d'
...
class iqIndicatorConstructorDlg(indicator_constructor_dlg_proto.iqIndicatorConstructorDlgProto,
    ...
    def setIndicatorListCtrl(self, indicator=None):
        ...
        for state_idx, state_indicator in enumerate(indicator):
            ...
            self.appendListCtrlRow(listctrl=self.indicator_listCtrl,
                                   row=('', ''))
    ...
    def initIndicatorGrid(self):
        ...
        self.setListCtrlColumns(listctrl=self.indicator_listCtrl,
                                cols=(dict(label=u'Name', width=200),
    ...
    def setDefaultStateCtrlValue(self):
        """
        Set default values for controls of the indicator state editor.
        """
    ...
    def refreshStateRow(self, state_idx, state_indicator):
        """
        Refresh the indicator list string for the corresponding state.

        :param state_idx: Index in the status list.
        :param state_indicator: Dictionary of indicator state description.
            {
                'name': State name,
                'image': Image filename,
                'text_color': Text colour tuple (R, G, B),
        ...
        if img_filename:
            if os.path.exists(img_filename):
                ...
                # In all other cases, we assume that this is a library file
        ...
        #                       row=(name, line))
        self.setListCtrlRow(listctrl=self.indicator_listCtrl,
                            item=state_idx, row=(name, line))
        if image:
            ...
            self.setListCtrlItemImage(listctrl=self.indicator_listCtrl,
                                      item=state_idx, img_name=image)

        if text_color:
            # log_func.debug(u'Text colour: %s' % str(text_color))
        ...
        if bg_color:
            # log_func.debug(u'Background colour: %s' % str(bg_color))
    ...
    def onDelToolClicked(self, event):
        """
        Handler for the button to delete the indicator state.
        """
        event.Skip()
    ...
    def onSaveToolClicked(self, event):
        ...
        try:
            ...
            idx = self.getListCtrlSelectedRowIdx(listctrl_or_event=self.indicator_listCtrl)
    ...
    def onTextColorCheckBox(self, event):
        """
        Handler for on / off the indicator status text color.
        """
        checkbox_state = event.IsChecked()
