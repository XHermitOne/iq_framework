...
class iqFilterTreeCtrlProto(wx.TreeCtrl,
    ...
    def editRootFilter(self, root_item=None):
        ...
        try:
            ...
            cur_filter = filter_choicectrl.getFilterChoiceDlg(parent=self,
    ...
    def refreshRootItemTitle(self):
        ...
        item_data = self.getTreeCtrlItemData(treectrl=self, item=item)
        cur_filter = item_data.get('__filter__', None)
        label = filter_choicectrl.getFilterAsStr(cur_filter) if cur_filter else DEFAULT_ROOT_LABEL
        if not label:
    ...
    def saveFilters(self, save_filename=None):
        ...
        if save_filename is None:
            save_filename = self.getSaveFilename()

        if save_filename:
            save_filename = os.path.normpath(save_filename)
        else:
            widget_uuid = self.getGUID()
            save_filename = os.path.join(file_func.getProjectProfilePath(),
                                         widget_uuid + '.dat')

        filter_tree_data = self.getTreeCtrlData(treectrl=self)
        result = self.saveCustomData(save_filename=save_filename, save_data=filter_tree_data)
        log_func.info(u'Save filter tree control <%s> filters to <%s>' % (self.getName(), save_filename))
        return result
    ...
    def loadFilters(self, save_filename=None):
        ...
        if not os.path.exists(save_filename):
            log_func.warning(u'Filter tree controller filters <%s> not found' % save_filename)

        filter_tree_data = self.loadCustomData(save_filename=save_filename)
        if filter_tree_data:
            log_func.info(u'Load filter tree control <%s> from <%s>' % (self.getName(), save_filename))
            # Build tree
