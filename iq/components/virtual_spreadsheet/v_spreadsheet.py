...
class iqVSpreadsheet(v_prototype.iqVPrototype):
    ...
    def __init__(self, encoding='utf-8', *args, **kwargs):
        ...
        self._data = {'name': 'Excel', '_children_': []}
    ...
    def saveAsXML(self, xml_filename=None):
        ...
        save_data = self.getData()['_children_'][0]
        try:
    ...
    def getWorkbook(self, name=None):
        ...
        """
        if name is None:
            if self._data['_children_']:
                attrs = [element for element in self._data['_children_'] if element['name'] == 'Workbook']
                if attrs:
                    work_book = v_workbook.iqVWorkbook(self)
    ...
    def _findWorksheetData(self, xml_filename=None, sheet_name=None):
        ...
        worksheet_data = [data for data in workbook_data['_children_'] if data['name'] == 'Worksheet']
        if worksheet_data:
    ...
    def _getWorkbookStyles(self, xml_filename=None):
        ...
        styles_data = [data for data in workbook_data['_children_'] if data['name'] == 'Styles']
        if not styles_data:
    ...
    def delWithoutSelectedWorksheetList(self, xml_filename=None):
        ...
        worksheet_name_list = [data['Name'] for data in workbook_data['_children_'] if data['name'] == 'Worksheet']
    ...
    def _pasteStyleIntoWorkbook(self, style, workbook_data):
        ...
        """
        try:
            workbook_styles = [data for data in workbook_data['_children_'] if data['name'] == 'Styles'][0]
            if style not in workbook_styles['_children_']:
                workbook_styles_id = [data['ID'] for data in workbook_styles['_children_'] if data['name'] == 'Style']

                new_style_id = self._genNewStyleID(style['ID'], workbook_styles_id)
                style['ID'] = new_style_id
                workbook_styles['_children_'].append(style)
                return new_style_id
            else:
    ...
    def _replaceStyleID(self, data, old_style_id, new_style_id):
        ...
        if '_children_' in data and data['_children_']:
            for i, child in enumerate(data['_children_']):
                data['_children_'][i] = self._replaceStyleID(child, old_style_id, new_style_id)
        return data
    ...
    def pasteWorksheet(self, xml_filename=None, is_cut=None, new_worksheet_name=None):
        ...
        sheet_names = [data['Name'] for data in workbook_data['_children_'] if data['name'] == 'Worksheet']
        ...
        if 'styles' in self._worksheet_clipboard:
            for i, style in enumerate(self._worksheet_clipboard['styles']['_children_']):
        ...
        # Insert data
        workbook_data['_children_'].append(worksheet_data)
    ...
    def setCellStyle(self, sheet_name, row, col, alignment=None,
        ...
        borders = {'name': 'Borders', '_children_': []}
        if left_border:
            left_border['Position'] = 'Left'
            borders['_children_'].append(left_border)
        if right_border:
            right_border['Position'] = 'Right'
            borders['_children_'].append(right_border)
        if top_border:
            top_border['Position'] = 'Top'
            borders['_children_'].append(top_border)
        if bottom_border:
            bottom_border['Position'] = 'Bottom'
            borders['_children_'].append(bottom_border)
