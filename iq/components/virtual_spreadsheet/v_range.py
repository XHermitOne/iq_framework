...
class iqVRange(v_prototype.iqVPrototype):
    ...
    def _isBorderPosition(self, style, border_position):
        ...
        """
        return bool([border for border in style['_children_'] if border['Position'] == border_position])
    ...
    def _getCellBorderAttrIdx(self, old_borders, i_row, i_col,
        ...
        """
        if old_borders and '_children_' in old_borders:
            attrs = {'borders': {'name': 'Borders', '_children_': copy.deepcopy(old_borders['_children_'])}}
        else:
            attrs = {'borders': {'name': 'Borders', '_children_': []}}
        ...
        if i_row == 0:
            ...
            if border_top:
                if not self._isBorderPosition(attrs['borders'], 'Top'):
                    ...
                    attrs['borders']['_children_'].append(cur_border)
            if i_col == 0:
                # Upper left cell
                if border_left:
                   ...
                   (i_row == self._height_1) or (i_col == self._width_1):
                    ...
                    if style_id is not None:
                        ...
                        if style:
                            ...
                            style_attrs = {'borders': {'name': 'Borders', '_children_': []}}
                    else:
                        style_attrs = {'borders': {'name': 'Borders', '_children_': []}}
    ...
    def copy(self):
        ...
        for i_row in range(self.height):
            cur_row = {'name': 'Row', '_children_': []}
            copy_result['_children_'].append(cur_row)

            cell = None
            for i_col in range(self.width):
                cell = self._parent.getCell(self.row+i_row, self.col+i_col)
                cell_attrs = copy.deepcopy(cell.getAttributes())
                cur_row['_children_'].append(cell_attrs)
...
class iqVColumn(v_prototype.iqVIndexedPrototype, iqVRange):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self._attributes = {'name': 'Column', '_children_': []}
...
class iqVRow(v_prototype.iqVIndexedPrototype, iqVRange):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        v_prototype.iqVIndexedPrototype.__init__(self, parent, *args, **kwargs)
        iqVRange.__init__(self, parent, *args, **kwargs)
        self._attributes = {'name': 'Row', '_children_': []}
...
#         self._attributes['Index']=text(index)
    ...
    def insertCellIdx(self, cell, idx):
        ...
        if cell_attr is None:
            cell.setIndex(idx)
        self._attributes['_children_'].insert(ins_i, cell.getAttributes())
        return cell
    ...
    def createCellIdx(self, idx):
        ...
        self._attributes['_children_'] = self._attributes['_children_'][:-1]
