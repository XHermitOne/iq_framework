...
class iqVStyles(v_prototype.iqVPrototype):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self._attributes = {'name': 'Styles', '_children_': []}
        self._style_dict = {}
    ...
    def getStyle(self, style_id):
        ...
        if style_id in self._style_dict:
            ...
            find_style = [style_attr for style_attr in self._attributes['_children_']
    ...
    def _equalStyleElement(self, style_element_attr1, style_element_attr2):
        ...
        if equal:
            if ('_children_' in style_element_attr1 and style_element_attr1['_children_']) or \
               ('_children_' in style_element_attr2 and style_element_attr2['_children_']):

                children1 = style_element_attr1['_children_']
                children2 = style_element_attr2['_children_']
    ...
    def _compareStyleElement(self, element, element_name, style):
        ...
        """
        if element:
            elements = [el for el in style['_children_'] if el['name'] == element_name]
            if elements:
    ...
    def findStyle(self, alignment=None,
        ...
        if not find_result:
            # Find style
            for i in range(len(self._attributes['_children_'])):
                style = self._attributes['_children_'][-i]
                # If the number of style elements does not match
                # the number of elements you are looking for,
                # then these styles are not mask
                if len(style['_children_']) != len([element for element in [alignment, borders, font, interior,
    ...
    def _getCrcAlignmentStr(self, style):
        ...
        """
        align = [element for element in style['_children_'] if element['name'] == 'Alignment']
        if align:
    ...
    def _getCrcBordersStr(self, style):
        ...
        """
        borders = [element for element in style['_children_'] if element['name'] == 'Borders']
        if borders:
            borders_str = ''
            for border in borders[0]['_children_']:
    ...
    def _getCrcFontStr(self, style):
        ...
        """
        font = [element for element in style['_children_'] if element['name'] == 'Font']
        if font:
    ...
    def _getCrcInteriorStr(self, style):
        ...
        """
        interior = [element for element in style['_children_'] if element['name'] == 'Interior']
        if interior:
    ...
    def _getCrcNumberFormatStr(self, style):
        ...
        """
        num_fmt = [element for element in style['_children_'] if element['name'] == 'NumberFormat']
        if num_fmt:
    ...
    def _calcCrcStyleAttr(self, alignment=None,
        ...
        """
        new_style_attr = {'_children_': []}
        if alignment:
            alignment['name'] = 'Alignment'
            new_style_attr['_children_'].append(alignment)
        if borders:
            borders['name'] = 'Borders'
            new_style_attr['_children_'].append(borders)
        if font:
            font['name'] = 'Font'
            new_style_attr['_children_'].append(font)
        if interior:
            interior['name'] = 'Interior'
            new_style_attr['_children_'].append(interior)
        if number_format:
            number_format['name'] = 'NumberFormat'
            new_style_attr['_children_'].append(number_format)
    ...
    def getStylesID(self):
        ...
        """
        return [style['ID'] for style in self._attributes['_children_']]
    ...
    def _createDefaultStyle(self):
        ...
        """
        styles_id = [style_element['ID'] for style_element in self._attributes['_children_']]
    ...
    def clearUnUsedStyles(self):
        ...
        work_sheets = [element for element in self._parent._attributes['_children_'] if element['name'] == 'Worksheet']
        for work_sheet in work_sheets:
            tables = [element for element in work_sheet['_children_'] if element['name'] == 'Table']
            for table in tables:
                if 'StyleID' in table:
                    style_id = table['StyleID']
                    if style_id not in used_styles_id:
                        used_styles_id.append(style_id)
                for tab_element in table['_children_']:
                    if 'StyleID' in tab_element:
                        style_id = tab_element['StyleID']
                        if style_id not in used_styles_id:
                            used_styles_id.append(style_id)
                    if tab_element['name'] == 'Row':
                        for cell in tab_element['_children_']:
                            if 'StyleID' in cell:
    ...
    def delStyleByID(self, style_id):
        ...
        if style_idx >= 0:
            del self._attributes['_children_'][style_idx]
            return True
        return False
...
class iqVStyle(v_prototype.iqVPrototype):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self._attributes = {'ID': self.newID(), 'name': 'Style', '_children_': []}
    ...
    def _delAttr(self, name):
        ...
        """
        try:
            idx = [attr['name'] for attr in self._attributes['_children_']].index(name)
        except ValueError:
            idx = -1
        if idx >= 0:
            del self._attributes['_children_'][idx]
    ...
    def updateAttrs(self, alignment=None,
        ...
        if update_attrs is None:
            style_attrs = self._attributes['_children_']
        else:
        ...
        self._attributes['_children_'] = style_attrs
    ...
    def getAttrs(self):
        ...
        """
        attrs = {}
        for element in self._attributes['_children_']:
...
class iqVBorders(v_prototype.iqVPrototype):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        """
        v_prototype.iqVPrototype.__init__(self, parent, *args, **kwargs)
        self._attributes = {'name': 'Borders', '_children_': []}
