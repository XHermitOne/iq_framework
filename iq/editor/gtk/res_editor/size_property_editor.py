...
class iqSizePropertyEditor(gtk_handler.iqGtkHandler,
                           property_editor_proto.iqPropertyEditorProto):
    """
    Size property editor class.
    """
    def __init__(self, label='', value=None, choices=None, default=None, *args, **kwargs):
        self.glade_filename = os.path.join(os.path.dirname(__file__), 'size_property_editor.glade')
    ...
    def setValue(self, value):
        ...
        if isinstance(value, (list, tuple)) and len(value) == 2:
            width, height = value
            self.getGtkObject('width_spinbutton').set_value(width)
            self.getGtkObject('height_spinbutton').set_value(width)

    def getValue(self):
        """
        Get value.
        """
        return self.getGtkObject('width_spinbutton').get_value(), self.getGtkObject('height_spinbutton').get_value()
