...
from . import mainform_generator
    ...
    try:
        ...
        if src_class_name is None:
            if len(fb_module_classes) == 1:
                ...
                choices.sort()
                src_class_name = dlg_func.getSingleChoiceDlg(parent=parent,
                                                             title=u'GENERATOR',
...
def gen_mainform(src_filename=None, dst_filename=None, src_class_name=None, parent=None, rewrite=False):
    """
    Generation of the main form module modulo the form generated by wxFormBuilder.
    ...
    try:
        ...
        if src_class_name is None and fb_module_classes:
            src_class_name = fb_module_classes[0]
            if len(fb_module_classes) > 1:
                log_func.warning(u'Multiple classes to generate. Selected <%s> class' % src_class_name)

        if dst_filename is None:
            # If the output file name is not defined, then generate the file name from the form class name
            dst_module_name = genPyModuleName(src_class_name)
            dst_filename = os.path.join(src_module_path,
                                        '%s.py' % dst_module_name)
        ...
        if not os.path.exists(dst_filename):
            if fb_module:
                ...
                py_txt = None
                if issubclass(src_class, wx.Frame):
                    py_txt = mainform_generator.genPythonFrame(fb_module, src_class_name)
                else:
                    log_func.warning(u'Unsupported type for generating python module <%s>' % src_class.__name__)
        ...
        log_func.fatal(u'Error generation of the frame module modulo the main form generated by wxFormBuilder')
