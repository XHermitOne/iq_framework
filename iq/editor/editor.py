...
from ..editor.runtui import form_json_manager
...
def _openResourceEditorWX(res_filename):
    """
    WX engine. Open resource editor form.
...
def _openResourceEditorGTK(res_filename):
    """
    GTK engine. Open resource editor form.
...
def _openResourceEditroRUNTUI(res_filename):
    """
    RUNTUI engine.Open resource editor form.
    ...
    if os.path.isdir(res_filename) and res_filename == file_func.getFrameworkPath():
        log_func.info(u'Main editor <%s>' % res_filename)
        return openFrameworkEditor()
    elif form_json_manager.isFormJSONProjectFile(res_filename):
        log_func.info(u'Edit JSON RunTUI form project <%s>' % res_filename)
        from .runtui import start_designer
        return start_designer.startRunTUIRadDesigner(json_filename=res_filename)
    elif os.path.isdir(res_filename):
        log_func.info(u'Edit folder <%s>' % res_filename)
        from .runtui import start_folder
        return start_folder.startFolder(folder_path=res_filename)
    else:
        log_func.warning(u'Not support editing file <%s>' % res_filename)
    return False
...
def _openResourceEditor(res_filename):
    ...
    if global_func.isWXEngine():
        return _openResourceEditorWX(res_filename=res_filename)
    elif global_func.isGTKEngine():
        return _openResourceEditorGTK(res_filename=res_filename)
    elif global_func.isRUNTUIEngine():
        return _openResourceEditroRUNTUI(res_filename=res_filename)
    else:
        log_func.warning(u'Not supported engine <%s> as editor' % global_func.getEngineType())
    return False
